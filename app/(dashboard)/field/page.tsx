...
async function FieldContent() {
  ...
  const dateLabel = new Date().toLocaleDateString(locale === 'en' ? 'en-CA' : 'fr-CA', { weekday: 'long', day: 'numeric', month: 'long' })
  ...
  return (
    <div className="mx-auto max-w-md space-y-5">
      ...
      <div className="rounded-2xl bg-neutral-900 p-5 text-white dark:bg-neutral-100 dark:text-neutral-900">
        ...
        <h1 className="mt-1 text-xl font-bold">{T('field.hello')}{firstName ? ` ${firstName}` : ''} 👋</h1>
        <div className="mt-3 flex gap-4">
          <div><p className="text-2xl font-bold">{events.length}</p><p className="text-xs opacity-70">{T('field.jobsCount')}</p></div>
          <div><p className="text-2xl font-bold">{hoursToday.toFixed(1)} h</p><p className="text-xs opacity-70">{T('field.hoursToday')}</p></div>
        </div>
        {!employee && <p className="mt-3 text-[11px] opacity-70">{T('field.dispatchNote')}</p>}
      </div>
      ...
      {events.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-neutral-200 py-14 text-center dark:border-neutral-800">
          ...
          <p className="text-sm font-medium text-neutral-700 dark:text-neutral-300">{T('field.noJobs')}</p>
          <p className="mt-1 text-xs text-neutral-400">{T('field.noJobsSub')}</p>
        </div>
      ) : (
              ...
              <div key={e.id} className="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm dark:border-neutral-800 dark:bg-neutral-950">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <span className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-medium ${isOpen ? 'bg-emerald-100 text-emerald-700' : 'bg-neutral-100 text-neutral-600 dark:bg-neutral-800 dark:text-neutral-300'}`}>
                    {isOpen ? T('field.inProgress') : (STATUS_LABEL[e.status] ?? e.status)}
                  </span>
                </div>
                ...
                <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-neutral-500 dark:text-neutral-400">
                  ...
                  {e.estimated_hours != null && <span>{T('field.hoursPlanned', { n: e.estimated_hours })}</span>}
                  {maps && <a href={maps} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-neutral-600 underline-offset-2 hover:underline dark:text-neutral-300"><MapPin className="h-3.5 w-3.5" />{T('field.route')}</a>}
                </div>
                ...
                <div className="mt-3 flex flex-wrap items-center gap-2">
                  {e.project_id ? (
                    ...
                    <span className="text-xs text-neutral-400">{T('field.noClock')}</span>
                  )}
                  {e.project_id && (
                    <>
                      <FieldPhotoButton projectId={e.project_id} />
                      <FieldReportSheet projectId={e.project_id} projectTitle={e.project?.title ?? e.title} />
                      <Link href={`/projects/${e.project_id}`} className="inline-flex h-11 items-center gap-1 rounded-full px-3 text-sm font-medium text-neutral-500 hover:text-neutral-900 dark:hover:text-neutral-100">{T('field.project')}<ArrowRight className="h-4 w-4" /></Link>
                    </>
                  )}
                </div>
              </div>
      ...
      <div className="flex justify-center pt-1 text-xs">
        <Link href="/schedule" className="text-neutral-500 hover:text-neutral-900 dark:hover:text-neutral-100">{T('field.viewCalendar')}</Link>
      </div>
    </div>
  )
}

...
function win(label: string, wIn: number, hIn: number): ReportSurface {
  const w = ft(wIn), h = ft(hIn)
  return {
    id: id(), facade_side: 'front', surface_type: 'window', label,
    gross_area: w * h, opening_area: 0, net_area: w * h,
    perimeter: null, length: w, height: h, pitch: null, unit: 'ft', loss_factor: 0.1,
  }
}
function door(label: string, wIn: number, hIn: number): ReportSurface {
  const w = ft(wIn), h = ft(hIn)
  return {
    id: id(), facade_side: 'front', surface_type: 'door', label,
    gross_area: w * h, opening_area: 0, net_area: w * h,
    perimeter: null, length: w, height: h, pitch: null, unit: 'ft', loss_factor: 0.1,
  }
}
...
const surfaces: ReportSurface[] = [
  ...
  // Fenêtres
  win('W-105', 19, 55), win('W-106', 19, 55), win('W-107', 19, 55),
  win('W-108', 19, 55), win('W-109', 19, 55), win('W-110', 19, 55),
  win('W-111', 12, 44), win('W-112', 12, 44), win('W-113', 27, 14),
  win('W-114', 12, 44), win('W-115', 12, 44), win('W-116', 27, 14),
  win('W-117', 40, 36), win('W-118', 27, 46),
  win('W-001', 32, 21), win('W-002', 32, 21), win('W-003', 35, 17), win('W-004', 37, 19),
  // Portes
  door('D-1', 32, 80), door('SGD-1', 53, 70),
]
...
test('génère le rapport PDF complet (style Hover)', async () => {
  ...
  writeFileSync(resolve(OUT, 'rapport-mesures-complet.pdf'), buffer)
